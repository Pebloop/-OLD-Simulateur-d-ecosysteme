...
======================================================================================================================================================================================================*/
  ...
  final private String MAP_PATH = "ressources/map animaux/map_Renard.png";             //Chemin vers la carte de présence de renard sur la terre
  final private PImage ICONE = icone_Renard;                                           //ID de la PImage de l'icone de slection
  final private int[] BIOME_ADAPT = {4,0,1,0,0,-1,4,2,4,3,-30};                        //Tableau des valeurs de l'adaptation aux biome [0-10]
  final private String[] PREY_LIST = {"lapin","caille","grenouille","ecureil","canard"};                                        //Liste des proies de l'espèce
  final private int CONCENTRATION_MIN = 1;                                             //Concentration minimale de l'animal au km²
  final private int CONCENTRATION_MAX = 6;                                             //Concentration maximale de l'animal au km²
  final private int SANTE = 30;                                                        //Résistance à l'habitat
  final private int ENFANT_MIN = 4;                                                    //Nombre minimale d'enfants par portée
  final private int ENFANT_MAX = 6;                                                    //Nombre maximal d'enfants pas portée
  
  final private int DEFINE_VITESSE_PROPAGATION = 1;                                    //vitesse de propagation en pourcentage lors de la migration mensuelle
  final private int DEFINE_DUREE_VIE = 6;                                              //Durée de vie en année maximale
  final private int DEFINE_VIEILLESSE_MIN = 3;                                         //Durée de vie minimale
  final private String DEFINE_NOM_ESPECE = "Renard";                                   //Nom de l'espèce
...
======================================================================================================================================================================================================*/
   ...
   Renard(){ 
     ...
     icone = ICONE;
     gestation = new int[altitude.length][altitude[0].length][3]; 
...
//------------------------------------------------------------------------------------------------FONCTION----------------------------------------------------------------------------------------------
  ...
  private void Breed(int x,int y){                                                           //algorythme de reproduction
      if((mois == 12 || mois == 1 || mois == 2) && jour == 1)
        for(int i =0;i<(posMap[x][y]-(gestation[x][y][0]+gestation[x][y][1]+gestation[x][y][2]))/2;i++){
