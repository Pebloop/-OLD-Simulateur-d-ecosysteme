...
======================================================================================================================================================================================================*/
  ...
  final private String MAP_PATH = "ressources/map animaux/map_Lapin.png";             //Chemin vers la carte de présence de renard sur la terre
  final private PImage ICONE = icone_Lapin;                                           //ID de la PImage de l'icone de slection
  final private int[] BIOME_ADAPT = {4,0,1,0,0,1,4,2,4,3,0};                        //Tableau des valeurs de l'adaptation aux biome [0-10]
  ...
  final private int CONCENTRATION_MAX = 1000;                                             //Concentration maximale de l'animal au km²
  final private int SANTE = 30;                                                        //Résistance à l'habitat
  final private int ENFANT_MIN = 3;                                                    //Nombre minimale d'enfants par portée
  final private int ENFANT_MAX = 12;                                                    //Nombre maximal d'enfants pas portée
  
  final private int DEFINE_VITESSE_PROPAGATION = 1;                                    //vitesse de propagation en pourcentage lors de la migration mensuelle
  final private int DEFINE_DUREE_VIE = 8;                                              //Durée de vie en année maximale
  final private int DEFINE_VIEILLESSE_MIN = 4;                                         //Durée de vie minimale
  final private String DEFINE_NOM_ESPECE = "Lapin";                                   //Nom de l'espèce
...
======================================================================================================================================================================================================*/
   ...
   Lapin(){ 
     ...
     icone = ICONE;
     gestation = new int[altitude.length][altitude[0].length][7]; 
    ...
    for(int j = 0; j < nbreAnimal[0].length ; j++){
      ...
      println(random + " x " + animalParCase + " = " + animalParCase*random);
...
//------------------------------------------------------------------------------------------------FONCTION----------------------------------------------------------------------------------------------
 ...
 private void Breed(int x,int y){                                                           //algorythme de reproduction
      if((mois >= 2 && mois <= 8) && jour == 1)
        for(int i =0;i<(posMap[x][y]-(gestation[x][y][0]+gestation[x][y][1]+gestation[x][y][2]+gestation[x][y][3]+gestation[x][y][4]+gestation[x][y][5]+gestation[x][y][6]))/2;i++){
